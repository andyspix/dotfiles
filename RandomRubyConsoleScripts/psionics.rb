...
rep = [%w(Name LikelyLevel Cost Rating Duration Range Target ManifestTime Display Book Base? Discipline? Mantle? PsyWar? PsyRogue? Lurk? Medic? Level Description)]
lotsa_data.values.each { |a| rep << [a[:name], a[:likely_level], a[:cost], a[:rating], a[:duration], a[:range], a[:target], a[:mft_time], a[:display], a[:book], a[:base], a[:discipline], a[:mantle], a[:psywar], a[:psyrogue], a[:lurk], a[:medic], a[:level], a[:description] ] }
